...
export default async function IssuerPage() {
  ...
  if (token) {
    try {
      diplomas = await apiGet<DiplomaListItem[]>("/api/v1/issuer/diplomas", `access_token=${token}`);
    } catch {}
    try {
      profile = await apiGet<UniversityInfo>("/api/v1/issuer/profile", `access_token=${token}`);
    } catch {}
  }
