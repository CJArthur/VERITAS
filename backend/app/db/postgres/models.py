...
from sqlalchemy import (
    String, Text, Date, Integer, Numeric, 
...
class University(Base):
    ...
    users: Mapped[List["UniversityUser"]] = relationship(back_populates="university")
...
class Diploma(Base):
    ...
    graduate_full_name: Mapped[str] = mapped_column(Text)
    ...
    certificate_token: Mapped[UUID] = mapped_column(default=uuid4)
    
    status: Mapped[DiplomaStatus] = mapped_column(Enum(DiplomaStatus), default=DiplomaStatus.active)
...
class TranscriptSubject(Base):
    ...
    grade: Mapped[int] = mapped_column(Integer) # Можно усложнить для pass/fail
...
class VerificationLog(Base):
    ...
    verifier_type: Mapped[str] = mapped_column(String) # Либо Enum (employer, qr_scan, api)
...
class UniversityUser(Base):
    __tablename__ = "university_users"
