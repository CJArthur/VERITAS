...
def _signing_key_for_issuer(issuer_id: UUID) -> bytes:
    return hashlib.sha256(
        f"{SECRET_KEY}:{issuer_id}".encode("utf-8"),
    ).digest()
...
def issuer_signature_hmac(issuer_id: UUID, data_hash_hex: str) -> str:
    key = _signing_key_for_issuer(issuer_id)
    return hmac.new(key, data_hash_hex.encode("ascii"), hashlib.sha256).hexdigest()


def verify_issuer_signature(
    issuer_id: UUID, data_hash_hex: str, signature_hex: str
) -> bool:
    expected = issuer_signature_hmac(issuer_id, data_hash_hex)
