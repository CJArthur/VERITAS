...
> **Платформы без исходящего SMTP (Railway, Render и др.):** установите `SKIP_EMAIL_VERIFICATION=true`. В этом режиме аккаунт активируется мгновенно — без письма. Для продакшна верните `false` и настройте SMTP или HTTP-почтовый провайдер (Resend, SendGrid).
...
| `SKIP_EMAIL_VERIFICATION` | `false` по умолчанию. Установить `true` если SMTP недоступен (Railway и др. платформы, блокирующие исходящий SMTP). При `true` аккаунт верифицируется сразу после регистрации без отправки письма. |
...
По умолчанию после регистрации пользователь получает письмо со ссылкой активации. Аккаунт становится активным только после перехода по ней.

### Проблема: Railway и другие PaaS блокируют SMTP

Railway, Render и ряд других платформ блокируют исходящие соединения на порты 25, 465 и 587 (SMTP) в целях безопасности. Это означает, что письма не будут доставлены вне зависимости от правильности настроек почтового сервера.

### Решение: переменная `SKIP_EMAIL_VERIFICATION`

```env
# Для деплоя на Railway / без SMTP:
SKIP_EMAIL_VERIFICATION=true

# Для продакшна с нормальным SMTP:
SKIP_EMAIL_VERIFICATION=false  # (значение по умолчанию)
```

| Значение | Поведение |
|---|---|
| `false` | Стандартный флоу: письмо → ссылка → активация аккаунта |
| `true` | Аккаунт активируется сразу при регистрации, письмо не отправляется |

> Режим `true` предназначен для демо и разработки. В продакшне рекомендуется использовать `false` с настроенным SMTP или HTTP-почтовым провайдером (Resend, SendGrid, Mailgun), которые работают через HTTPS и не зависят от SMTP-портов.
